...
```{r initialLoad}
# Load in dataset
originalDf = read.csv('freshly_test_data.csv')
...
# A simple cleanliness check to see if any users are duplicated. We can group the data by user_id and counting the number of rows for each.
originalDf %>% 
  group_by(user_id) %>%
  summarize(count = n()) %>%
  arrange(desc(count)) %>%
...
renderUI({
  ...
  list(
      ...
      fluidRow(
        column(6, renderPlot(plot1)),
        column(6, renderPlot(plot2))
      )
  )
})
...
To understand which variables are predictive of lifetime value, it's important to

Linear Regression allows for very interpretable results
...
round(cor(lmDf), 3)
```
We use the lifetime_value column to see what independent variables display some correlation. We see that avg_rating and avg_time_spent both show high values, which is a good sign. 
``` {r correlated_plots}
...
renderUI({
  list(
    column(6, renderPlot(plot_avg_rating)),
    column(6, renderPlot(plot_avg_time_spent))
  )
})
```
